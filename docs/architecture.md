...
- public / private の prefix 分離や marker object は無く、ファイルツリーには本物のファイルだけが存在する（例外: SPA の「新規フォルダ」操作は、以降 upload される file の親 prefix を可視化するために 0-byte の `.keep` object を作る。`.keep` は SPA 上非表示にする。この 1 例だけ marker object を許容し、他の marker は使わない）
...
admin 運用は STS 経由の role ではなく、s3.json に定義した root identity（access key / secret key）で行う。運用スクリプト（quota reconciler / TTL sweep / multipart 掃除 / 監査ログ整理）が root credentials を使い、通常のユーザー経路（Keycloak -> STS）とは独立させる。root credentials は運用ホスト内の secret 管理から読み、compose 環境変数で ops サービスに注入する（[operations.md](./operations.md)）。
...
- 寿命は STS session の残り時間に cap され（session が切れた presign は `X-Amz-Expires` が残っていても 403）、session は min(元 access token の exp 残り, `DurationSeconds`（900-43200s）, `sts.maxSessionLength`) で決まる。access token lifespan は 43200s（kura client の override）、`sts.maxSessionLength` は 12h なので、presign の実効上限は約 12 時間。発行時点で fresh な access token を取得してから STS を切る（[frontend](#frontendreference-spa) 参照）ため、認証からの経過時間にかかわらず発行から最大 12 時間まで有効な URL を作れる
