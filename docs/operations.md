...
- SeaweedFS 一式・kura 内側 nginx・ops（日次運用タスク。「日次運用タスク」参照）を compose で動かす。port は内側 nginx = 28080、SeaweedFS S3 = 28333。dev は docker、a012 (staging / production) は rootless podman-compose（下記「a012 (rootless podman) の追加要件」）
- 起動:
...
- **Lustre は subuid range (`/etc/subuid` に割り当てられた 222700000+) の uid の read / write / chown を拒否する**。container 内 process の uid が host uid（w3ddbjld = 2227）にマップされるように userns を組む必要がある:
  ...
  - nginx: image の default `user nginx;` (worker uid 101) は subuid range に落ちる。`docker-entrypoint.d/` に置く `05-strip-user-directive.sh` が `user root;` に置換して master と worker を共に container root (= host w3ddbjld) で走らせる。`user` directive を削除すると nginx は default で `nobody` (uid 65534) を worker にしてしまうため置換 (削除ではない)
...
- 内側 nginx の public download ログ:
  ...
  - ディレクトリは nginx worker が書けること。compose の `kura-init` サービス（`alpine:3.20`）が起動時に `install -d -m 0777 /var/log/kura` で用意し、nginx / ops はこれを `depends_on: service_completed_successfully` で待ってから起動する。`chown` ではなく world-writable にするのは、rootless podman + Lustre で subuid range の uid への chown が拒否されるため（「a012 (rootless podman) の追加要件」参照）。手動で用意する場合も `install -d -m 0777 <dir>` で揃える
...
5. CORS: gateway が preflight に応答し、SPA からの S3 API（PUT / multipart / presign PUT）がブラウザで通ること。`KURA_S3_ALLOWED_ORIGINS` は staging / production では SPA 配信 origin を明示する（`*` は test 専用）。gateway 側の `Access-Control-Allow-Headers` に AWS SDK v3 が付ける `amz-sdk-request` / `amz-sdk-invocation-id` が含まれていないと preflight で block される（`service-gateway-conf/nginx/conf.d/kura-s3.conf`）
